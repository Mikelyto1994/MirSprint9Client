...
const CheckoutForm = ({ totalAmount, onPaymentSuccess }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const payload = {
      ...
      body: JSON.stringify({ amount: totalAmount, paymentMethod: { id: paymentMethod.id }, userEmail: email }),
    };

    const response = await fetch('http://localhost:8080/api/checkouts', payload);
    if (!response.ok) {
